...
import link.rdcn.DataFrameInfo
import link.rdcn.TestProvider.{csvDir, dataProvider}
...
class DataStreamSourceTest {
  ...
  def testCsvWithHeader(): Unit = {
    ...
    val expectedOutput = Source.fromFile(Paths.get(csvDir, "data_1.csv").toString).getLines().toSeq.tail.toList.head
  ...
  def testExcel(): Unit = {
    ...
    val excelPath = new File(excelDataFrameInfo.path).getPath
    ...
    val iter = source.iterator
    val rows = iter.toList
    assertEquals(1, rows.size, "Iterator should contain 0 mock excel rows")
  }
