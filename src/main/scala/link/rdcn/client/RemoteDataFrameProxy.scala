...
import link.rdcn.operation.{FilterOp, FunctionWrapper, LimitOp, MapOp, OperationChain, SelectOp, SerializableFunction, SingleRowCall}
...
case class RemoteDataFrameProxy(operation: OperationChain, getRows: String => (StructType, ClosableIterator[Row])) extends DataFrame with Logging {
